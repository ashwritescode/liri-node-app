...
function myTweets(){
  var screenName = {screen_name: 'ohemgeeashuhley'};
  client.get('statuses/user_timeline', screenName, function(error, tweets, response) {
    if (!error) {
      for (var i=0; i<tweets.length; i++){
        var postDate = tweets[i].created_at;
        console.log('@ohemgeeashuhley ' + tweets[i].text + " Tweeted at: " + postDate);
        console.log('-------------------');

        // append info to log.txt[1
        fs.appendFile('log.txt', '@ohemgeeashuhley: ' + tweets[i].text + ' Tweeted At: ' + postDate.substring(0, 19) + '\r\n'
          + '-------------------------------------------------------------' + '\r\n');
      }
    }
    else{
...
function spotifySong(s){
  spotify.search({ type: 'track', query: s }, function( error, data) {
    console.log(s);
    if(s == ''){
      console.log('Artist: Chainsmokers')
    } else{
      console.log('I wanna be like Kanyyeeee');
      if( !error ){
